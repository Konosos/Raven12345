...
- Sau khi import spine-unity, mở `Spine > Asset Studio` và nhấn **Detect Spine and Enable SPINE_UNITY**. Công cụ sẽ kiểm tra runtime rồi tự thêm define cho build target hiện tại; Unity sẽ reload và bật đầy đủ công cụ. Package không dùng assembly reference cứng tới Spine, nên hoạt động với spine-unity import từ Asset Store hoặc package có tên assembly khác nhau.
